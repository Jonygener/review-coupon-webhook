...
def create_discount_code(email, product_variant_id, product_id, code):
...
def update_klaviyo_profile(email, discount_code):
    ...
    profile_id = data['data'][0]['id'] if data.get('data') else None
    if not profile_id:
...
def webhook():
    ...
    logging.debug("DEBUG raw POST data: %s", data)
    ...
    create_discount_code(email, variant_id, product_id, discount_code_value)
    update_klaviyo_profile(email, discount_code_value)

    return jsonify({"message": "Coupon generated and assigned."}), 200
